...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    UINavigationController *nc = [[UINavigationController alloc] initWithRootViewController:self.secondVC];
    
    self.splitVC = [[UISplitViewController alloc] init];
    self.splitVC.delegate = self;
    self.splitVC.viewControllers = @[firstVC, nc];
    
    self.window = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
    
    self.window.rootViewController = self.splitVC;
    
    [self showButtonIfNeededByDisplayMode:self.splitVC.displayMode];
...
- (void)showButtonIfNeededByDisplayMode:(UISplitViewControllerDisplayMode)displayMode {
    ...
    if (displayMode == UISplitViewControllerDisplayModePrimaryHidden) {
        self.secondVC.navigationItem.leftBarButtonItem = self.splitVC.displayModeButtonItem;
    }
}
